...
const items = {
  de: [
    { href: "/iphone-17", title: "iPhone 17", text: "Air, Pro und Pro Max", matches: (product: Product) => product.category === "smartphones" && normalizeProductBrand(product.brand) === "Apple" && /iphone\s*17/i.test(`${product.title} ${product.model ?? ""}`) },
    { href: "/iphone-16-pro-max", title: "iPhone 16 Pro Max", text: "Neu, Open Box & gebraucht", matches: (product: Product) => product.category === "smartphones" && normalizeProductBrand(product.brand) === "Apple" && /iphone\s*16\s*pro\s*max/i.test(`${product.title} ${product.model ?? ""}`) },
  ...
  en: [
    { href: "/iphone-17", title: "iPhone 17", text: "Air, Pro and Pro Max", matches: (product: Product) => product.category === "smartphones" && normalizeProductBrand(product.brand) === "Apple" && /iphone\s*17/i.test(`${product.title} ${product.model ?? ""}`) },
    { href: "/iphone-16-pro-max", title: "iPhone 16 Pro Max", text: "New, open box and used", matches: (product: Product) => product.category === "smartphones" && normalizeProductBrand(product.brand) === "Apple" && /iphone\s*16\s*pro\s*max/i.test(`${product.title} ${product.model ?? ""}`) },
...
export const getStoreCollectionPreviews = (lang: Locale, products: Product[]): StoreCollectionPreview[] => {
  ...
  return items[lang].map(item => {
    ...
    return { href: item.href, title: item.title, text: item.text, count: matches.length, preview: matches[0] };
  });
};
    ...
    <section className="border-b border-border bg-background py-5" aria-labelledby="shop-collections-heading">
      <div className="container-page">
        ...
        <div className="-mx-6 flex snap-x gap-3 overflow-x-auto px-6 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {getStoreCollectionPreviews(lang, products).map((item) => {
            ...
            return (
              <Link key={item.href} href={`/${lang}${item.href}`} className="group flex w-[184px] shrink-0 snap-start items-center gap-2.5 rounded-2xl border border-border bg-store-card p-2.5 transition hover:border-gold/45 hover:bg-gold/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold sm:w-[224px] sm:gap-3 sm:p-3">
                ...
                <span className="min-w-0">
                  ...
                  <span className="mt-1 block text-[11px] font-semibold text-muted-strong">{item.count} {lang === "de" ? (item.count === 1 ? "Angebot" : "Angebote") : (item.count === 1 ? "offer" : "offers")}</span>
                </span>
              </Link>
            );
          })}
