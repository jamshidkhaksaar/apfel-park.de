...
export default function WhatsAppFloat({ lang }: WhatsAppFloatProps) {
  ...
  return (
    ...
    >
      ...
      {/* Pulsing Ring */}
      <div className="absolute inset-0 animate-ping rounded-full bg-green-500/30" style={{ animationDuration: "2s" }} />
      <div className="absolute inset-0 animate-pulse rounded-full bg-green-500/20" style={{ animationDuration: "1.5s" }} />
      ...
      >
        ...
        <span className="absolute -right-1 -top-1 flex h-5 w-5 items-center justify-center rounded-full bg-gold text-[10px] font-bold text-contrast-adaptive shadow-lg">
          ...
          <span className="sr-only">{lang === "de" ? "1 ungelesene Nachricht" : "1 unread message"}</span>
        </span>
      </a>
    </div>
  );
