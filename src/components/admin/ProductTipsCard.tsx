  ...
  return (
    <section className="mx-auto mb-4 w-full max-w-[1500px] rounded-2xl border border-border/60 bg-surface/55 p-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="text-xs font-semibold uppercase tracking-[0.16em] text-foreground">{isGerman ? "Produkt-Tipps" : "Product tips"}</p>
          <p className="mt-1 text-sm text-foreground">{tips.items.length === 0 ? (isGerman ? "Alles vollständig." : "Everything complete.") : `${tips.items.length} ${isGerman ? "Punkte fehlen" : "items missing"}`}</p>
        </div>
      ...
      {tips.stockZero ? <p className="mt-2 text-xs font-semibold text-red">{isGerman ? "Ausverkauft – Bestand prüfen." : "Out of stock – check stock."}</p> : null}
    </section>
  );
}
