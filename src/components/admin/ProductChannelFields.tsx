    ...
    <section className="rounded-3xl border border-border/60 bg-surface/45 p-5">
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {(Object.keys(channelLabels) as Array<keyof typeof channelLabels>).map((channel) => {
          ...
          return (
            <div key={channel} className={`rounded-2xl border p-4 ${result.ready ? "border-emerald-500/30 bg-emerald-500/5" : "border-amber-500/30 bg-amber-500/5"}`}>
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-semibold text-foreground">{channelLabels[channel]}</span>
                <span className={`text-[10px] font-semibold uppercase tracking-[0.16em] ${result.ready ? "text-emerald-300" : "text-amber-300"}`}>
                  {result.ready ? (locale === "de" ? "Bereit" : "Ready") : (locale === "de" ? "Entwurf" : "Draft")}
                </span>
              </div>
              {result.errors.length > 0 ? <ul className="mt-3 space-y-1 text-xs text-amber-100">{result.errors.map((message) => <li key={message}>• {message}</li>)}</ul> : null}
              {result.warnings.length > 0 ? <ul className="mt-3 space-y-1 text-xs text-muted">{result.warnings.map((message) => <li key={message}>• {message}</li>)}</ul> : null}
            </div>
          );
        })}
      </div>
