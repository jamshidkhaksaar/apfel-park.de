...
type ViewMode = "unfolded" | "colors" | "profile" | "video";
...
/* -------------------------------------------------------------------------- */
/* Custom Hardware Architecture SVGs (Zero Emojis)                            */
/* -------------------------------------------------------------------------- */

// 1. Google Tensor G5 (3nm TSMC Silicon Die & TPU)
...
// 2. Pro Pixel Camera & Periscope Telephoto Optics
...
// 3. Fluid Frictionless Gear Hinge (Zero-Gap 180° Fold)
...
// 4. Super Actua OLED 1-120Hz LTPO Canvas
function SuperActuaDisplaySvg({ className = "size-16" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 120 120" fill="none" aria-hidden="true">
      ...
      {/* 1Hz to 120Hz dynamic frequency waveform */}
      <path
        ...
        strokeLinecap="round"
      />
      {/* Anti-reflective subpixel array representation */}
...
export default function PixelShowcase({
  ...
  const [viewMode, setViewMode] = useState<ViewMode>("unfolded");
  ...
  const getActiveAsset = () => {
    if (viewMode === "video") {
      return { type: "video" as const, src: currentVideo.src };
    }
    if (viewMode === "profile") {
      return {
        type: "image" as const,
        src: selectedModel === "proFold" ? "/images/google/pixel11/fold-obsidian.webp" : "/images/google/pixel11/pixel-obsidian.webp",
        alt: isDe
          ? "Google Pixel 11 Serie Seitenprofil & Gehäusefinish"
          : "Google Pixel 11 Series side profile and chassis build",
        labelDe: selectedModel === "proFold" ? "Pro Fold · 5,1 mm Slim Profil & Scharnier" : "Pixel 11 · 8,5 mm Unibody Profil",
        labelEn: selectedModel === "proFold" ? "Pro Fold · 5.1mm Slim Profile & Hinge" : "Pixel 11 · 8.5mm Unibody Profile",
      };
    }
    if (viewMode === "unfolded") {
      if (selectedModel === "proFold") {
        return {
          type: "image" as const,
          src: "/images/google/pixel11/fold-olive.webp",
          alt: isDe
          ...
          labelDe: "Pixel 11 Pro Fold · 8,0\" Super Actua Flex (1-120 Hz)",
          labelEn: "Pixel 11 Pro Fold · 8.0\" Super Actua Flex (1-120 Hz)",
        };
      }
      return {
        type: "image" as const,
        ...
        labelDe: "Pixel 11 · 6,3\" Actua OLED (120 Hz, 2.700 Nits)",
        labelEn: "Pixel 11 · 6.3\" Actua OLED (120 Hz, 2,700 Nits)",
      };
    }
    // "colors" mode
    return {
  ...
  const waText = encodeURIComponent(
    isDe
      ? `Guten Tag Apfel Park Team, ich interessiere mich für das Google ${
        ...
        } in der Farbe ${isDe ? currentFinish.nameDe : currentFinish.nameEn}. Bitte senden Sie mir ein Angebot und Infos zur Verfügbarkeit in Hamburg.`
  ...
  const faqs = [
    ...
    {
      qDe: "Wie dünn ist das Google Pixel 11 Pro Fold?",
      qEn: "How thin is the Google Pixel 11 Pro Fold?",
      aDe: "Das Pixel 11 Pro Fold misst entfaltet nur 5,1 mm und gefaltet 10,5 mm. Dank des reibungslosen Zahnradscharniers aus hochfestem Stahl schließt das Gerät völlig flach ab.",
      aEn: "Pixel 11 Pro Fold measures just 5.1mm unfolded and 10.5mm folded. Its fluid gear hinge made of high-strength steel ensures a completely flush, zero-gap closure.",
    },
    {
            ...
            <div>
              <div className="flex items-center gap-2">
                ...
                <span className="rounded-full bg-blue/15 px-2.5 py-0.5 text-[11px] font-semibold text-blue">
                  {selectedModel === "pixel11" ? "Tensor G5 3nm" : "8,0\" Super Actua Flex"}
                </span>
              </div>
              <p className="text-xs sm:text-sm text-muted mt-0.5">
                {isDe
                  ? "Offizielle Google Store Renderings, Farbvarianten und interaktive Videopräsentation"
                  : "Official Google Store renders, color finishes, and interactive video showcase"}
              </p>
            </div>
          ...
          <div className="relative overflow-hidden rounded-3xl border border-border bg-surface-strong/50 p-4 sm:p-8 flex flex-col items-center justify-center min-h-[460px] sm:min-h-[580px] md:min-h-[660px] lg:min-h-[720px]">
            ...
            {activeAsset.type === "video" ? (
              <div className="relative z-10 w-full max-w-4xl aspect-[16/9] overflow-hidden rounded-2xl bg-black border border-border shadow-2xl flex flex-col justify-between">
                ...
                {/* Playlist selector bar below video */}
            ...
            <table className="w-full text-left text-xs sm:text-sm">
              <thead>
                <tr className="border-b border-border bg-surface-strong/60">
                  ...
                  <th className={`p-4 font-bold w-1/3 ${selectedModel === "pixel11" ? "text-gold bg-gold/5" : "text-foreground"}`}>
                    Google Pixel 11 (6,3&quot;)
                  </th>
                  <th className={`p-4 font-bold w-1/3 ${selectedModel === "proFold" ? "text-gold bg-gold/5" : "text-foreground"}`}>
              ...
              <tbody className="divide-y divide-border/60">
                {specRows.map((row) => (
                  <tr key={row.category} className="transition-colors hover:bg-surface/50">
                    ...
                    <td className={`p-4 leading-relaxed ${selectedModel === "pixel11" ? "bg-gold/5 font-medium text-foreground" : "text-muted"}`}>
                      {row.pixel11}
                    </td>
                    <td className={`p-4 leading-relaxed ${selectedModel === "proFold" ? "bg-gold/5 font-medium text-foreground" : "text-muted"}`}>
                      {row.proFold}
                    </td>
                  </tr>
                ))}
