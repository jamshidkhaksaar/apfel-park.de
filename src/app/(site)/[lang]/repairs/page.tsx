...
import RepairPriceTable, { catalogueFloorPrices } from "../../../../components/RepairPriceTable";
...
export default async function RepairsPage({ params }: { params: Promise<{ lang: string }> }) {
  ...
  const floors = catalogueFloorPrices(repairCatalog);
  const repairOffers = [
    { key: "display" as const, de: "Display-Reparatur", en: "Display repair" },
    { key: "battery" as const, de: "Akkutausch", en: "Battery replacement" },
  ...
  const repairServiceJsonLd = {
    "@context": "https://schema.org",
    "@type": "Service",
    "@id": `${siteInfo.url}/${lang}/repairs#service`,
    serviceType: lang === "de" ? "Handy- und Smartphone-Reparatur" : "Phone and smartphone repair",
    name: lang === "de" ? "Handy Reparatur Hamburg" : "Phone Repair Hamburg",
    description: dict.meta.repairs.description,
    url: `${siteInfo.url}/${lang}/repairs`,
    provider: { "@id": `${siteInfo.url}/#store` },
    areaServed: [
      { "@type": "City", name: "Hamburg" },
      { "@type": "AdministrativeArea", name: "Hamburg-Wilhelmsburg" },
      { "@type": "AdministrativeArea", name: "Hamburg-Harburg" },
    ],
    hasOfferCatalog: {
      "@type": "OfferCatalog",
      name: lang === "de" ? "Reparaturleistungen" : "Repair services",
      itemListElement: repairOffers.map((offer) => ({
        "@type": "Offer",
        itemOffered: { "@type": "Service", name: lang === "de" ? offer.de : offer.en },
        priceSpecification: {
          "@type": "PriceSpecification",
          priceCurrency: "EUR",
          minPrice: floors[offer.key],
  ...
  return (
    <div className="bg-background">
      <script
        ...
        dangerouslySetInnerHTML={{ __html: JSON.stringify(repairServiceJsonLd) }}
      />
      <PageIntro
      ...
      <section className="section-pad bg-surface/20">
        <div className="container-page">
          ...
          <RepairPriceTable lang={lang} catalog={repairCatalog} />
        </div>
      </section>
