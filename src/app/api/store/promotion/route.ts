...
import { bannerCategories } from '@/lib/promotion';
...
export async function GET(request:NextRequest){
  ...
  if((category&&category!=='repairs'&&!(bannerCategories as readonly string[]).includes(category))||(slug&&(slug.length>200||!/^[a-zA-Z0-9_%.-]+$/.test(slug))))return NextResponse.json({promotion:null},{status:400,headers});
