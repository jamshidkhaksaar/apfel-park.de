...
export async function GET(request:NextRequest){
  ...
  if((category&&!(deviceCategories as readonly string[]).includes(category))||(slug&&(slug.length>200||!/^[a-zA-Z0-9_%.-]+$/.test(slug))))return NextResponse.json({promotion:null},{status:400,headers});
