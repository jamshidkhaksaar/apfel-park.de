...
import type { PrivacyContent, PrivacyLocaleContent, PrivacySection } from './page';
...
export default function PrivacyContentForm({ initialContent }: { initialContent: PrivacyContent }) {
  const [content, setContent] = useState<PrivacyContent>(initialContent);
  const [lang, setLang] = useState<Lang>('de');
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const set = (updater: (prev: PrivacyLocaleContent) => PrivacyLocaleContent) => {
  ...
  const updateSection = (index: number, field: keyof PrivacySection, value: string) => {
    set((p) => {
    ...
    <div className="space-y-6">
      ...
      <div className="tech-card rounded-xl p-5">
        <div className="mb-4 flex items-center justify-between">
          <p className={sectionTitleCls + ' mb-0'}>Privacy Sections</p>
          <button
            onClick={addSection}
