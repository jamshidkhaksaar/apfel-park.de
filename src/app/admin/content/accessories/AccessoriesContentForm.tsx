...
import type { AccessoriesContent, AccessoriesLocaleContent } from './page';
...
export default function AccessoriesContentForm({ initialContent }: { initialContent: AccessoriesContent }) {
  const [content, setContent] = useState<AccessoriesContent>(initialContent);
  const [lang, setLang] = useState<Lang>('de');
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const set = (updater: (prev: AccessoriesLocaleContent) => AccessoriesLocaleContent) => {
    ...
    <div className="space-y-6">
      ...
      <div className="tech-card rounded-2xl p-6">
        <div className="flex items-center justify-between mb-3">
          ...
          <button
            type="button"
            onClick={() => set((p) => ({ ...p, categories: [...p.categories, ''] }))}
          ...
          {loc.categories.map((category, i) => (
            <div key={i} className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => {
                  const categories = loc.categories.filter((_, idx) => idx !== i);
                  set((p) => ({ ...p, categories }));
                }}
