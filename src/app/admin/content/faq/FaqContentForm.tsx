...
import type { FaqContent, FaqItem, FaqLocaleContent } from './page';
...
export default function FaqContentForm({ initialContent }: { initialContent: FaqContent }) {
  const [content, setContent] = useState<FaqContent>(initialContent);
  const [lang, setLang] = useState<Lang>('de');
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const set = (updater: (prev: FaqLocaleContent) => FaqLocaleContent) => {
  ...
  const removeItem = (index: number) => {
    set((p) => ({ ...p, items: p.items.filter((_, i) => i !== index) }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="tech-card rounded-xl p-5">
        <div className="mb-4 flex items-center justify-between">
          <p className={sectionTitleCls + ' mb-0'}>FAQ Items</p>
          <button
            onClick={addItem}
