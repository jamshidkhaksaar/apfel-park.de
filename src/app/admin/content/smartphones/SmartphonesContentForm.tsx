...
import type { SmartphonesContent, SmartphonesLocaleContent } from './page';
...
export default function SmartphonesContentForm({ initialContent }: { initialContent: SmartphonesContent }) {
  const [content, setContent] = useState<SmartphonesContent>(initialContent);
  const [lang, setLang] = useState<Lang>('de');
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const set = (updater: (prev: SmartphonesLocaleContent) => SmartphonesLocaleContent) => {
