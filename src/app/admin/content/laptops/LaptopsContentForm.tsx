...
import type { LaptopsContent, LaptopsLocaleContent } from './page';
...
export default function LaptopsContentForm({ initialContent }: { initialContent: LaptopsContent }) {
  const [content, setContent] = useState<LaptopsContent>(initialContent);
  const [lang, setLang] = useState<Lang>('de');
  const [isPending, startTransition] = useTransition();
  const [message, setMessage] = useState<{ type: 'success' | 'error'; text: string } | null>(null);

  const set = (updater: (prev: LaptopsLocaleContent) => LaptopsLocaleContent) => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Sections */}
      <div className="tech-card rounded-2xl p-6">
        <p className={sectionTitleCls}>Sections</p>
        <div className="space-y-4">
          <div className="space-y-2 rounded-lg border border-white/10 p-3">
            <p className="text-xs text-muted font-medium">New Laptops</p>
            <div>
              <label className={labelCls}>Title</label>
              <input
                className={inputCls}
                value={loc.sections.new.title}
                onChange={(e) =>
                  set((p) => ({
                    ...p,
                    sections: { ...p.sections, new: { ...p.sections.new, title: e.target.value } },
                  }))
                }
              />
            </div>
            <div>
              ...
              <textarea
                className={textareaCls}
                value={loc.sections.new.subtitle}
                onChange={(e) =>
                  set((p) => ({
                    ...p,
                    sections: { ...p.sections, new: { ...p.sections.new, subtitle: e.target.value } },
                  }))
                }
              />
            </div>
          ...
          <div className="space-y-2 rounded-lg border border-white/10 p-3">
            <p className="text-xs text-muted font-medium">Refurbished Laptops</p>
            <div>
              <label className={labelCls}>Title</label>
              <input
                className={inputCls}
                value={loc.sections.refurbished.title}
                onChange={(e) =>
                  set((p) => ({
                    ...p,
                    sections: {
                      ...p.sections,
                      refurbished: { ...p.sections.refurbished, title: e.target.value },
                    },
                  }))
                }
              />
            ...
            <div>
              ...
              <textarea
                className={textareaCls}
                value={loc.sections.refurbished.subtitle}
                onChange={(e) =>
                  set((p) => ({
                    ...p,
                    sections: {
                      ...p.sections,
                      refurbished: { ...p.sections.refurbished, subtitle: e.target.value },
                    },
                  }))
                }
              />
      ...
      <div className="tech-card rounded-2xl p-6">
        <div className="flex items-center justify-between mb-3">
          <p className={sectionTitleCls}>Brands</p>
          <button
            type="button"
            onClick={() => set((p) => ({ ...p, brands: [...p.brands, ''] }))}
            className="text-xs text-gold hover:text-gold/80 transition"
          >
            + Add brand
          </button>
          ...
          {loc.brands.map((brand, i) => (
            <div key={i} className="flex gap-2">
              ...
              <button
                type="button"
                onClick={() => {
                  const brands = loc.brands.filter((_, idx) => idx !== i);
                  set((p) => ({ ...p, brands }));
                }}
                className="shrink-0 text-muted hover:text-red-400 transition text-xs px-2"
              >
