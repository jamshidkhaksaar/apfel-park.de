...
export const inventoryCatalogWhere = `WHERE ($1 = '' OR inventory.sku ILIKE $2 OR product.sku ILIKE $2 OR product.title ILIKE $2 OR coalesce(product.model, '') ILIKE $2)
  AND ($3 = 'all' OR ($3 = 'inventory' AND product.catalog_enabled = false)
    ...
    OR ($3 = 'needs_setup' AND (coalesce(product.is_active, false) = false
      OR inventory.id IS NOT NULL AND available_inventory(inventory.on_hand, inventory.reserved, inventory.safety_buffer) = 0
      OR inventory.id IS NULL AND coalesce(product.stock, 0) <= 0)))
