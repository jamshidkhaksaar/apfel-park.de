...
describe("buildStripePaymentReturnUrl", () => {
  it("returns the embedded payment to the correct localized order confirmation", () => {
    expect(
      ...
      "https://apfel-park.de/api/checkout/return/de?order_id=a7a617e2-d7bd-4d37-baaa-51a806523875&provider=stripe",
    );
  });
  ...
  it("does not depend on whether the origin has a trailing slash", () => {
    expect(
      ...
      "https://apfel-park.de/api/checkout/return/en?order_id=order-1&provider=stripe",
    );
  });
});
