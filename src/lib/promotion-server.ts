...
import { currentWeeklyEnd, normalizeWeeklyDays, upcomingWeeklyDates } from './campaign-weekly';
...
export const getPublicPromotion = async (context:{category?:string;slug?:string}={}): Promise<PublicPromotion|null> => {
  ...
  let {campaign,selectedCategories,selectedProducts}=await readPromotionCampaign(settings.campaignId);
  let fallback=false;
  if(!campaign||promotionCampaignIssue(campaign,selectedCategories)){
    // The selected offer may run only on Wednesday. Show the eligible weekly
    // offer on Monday or Tuesday without exposing one-off private coupons.
    const weekly=await query("SELECT id FROM store_campaigns WHERE is_active=true AND repair_rules ? 'weeklyDays' ORDER BY created_at ASC");
    for(const row of weekly.rows){
      const candidate=await readPromotionCampaign(String(row.id));
      if(!promotionCampaignIssue(candidate.campaign,candidate.selectedCategories)){
        ({campaign,selectedCategories,selectedProducts}=candidate);
        fallback=true;
        break;
      }
    }
  ...
  const dateEnd=weeklyDays.length?currentWeeklyEnd(weeklyDays):repairRules?.dates.length?campaignDateToIso(repairRules.dates.at(-1)+'T23:59:59'):null;
  ...
  return {repairRules,id:campaign.id,code:campaign.code,discountType:campaign.discount_type as 'percent'|'fixed',discountValue:Number(campaign.discount_value),minimumOrder:Number(campaign.minimum_order),
    startsAt:campaign.starts_at?new Date(campaign.starts_at).toISOString():null,endsAt,headline:fallback?{de:'',en:''}:settings.headline,
    upcomingDates:weeklyDays.length?upcomingWeeklyDates(weeklyDays,3):undefined,
