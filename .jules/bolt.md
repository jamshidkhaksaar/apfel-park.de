...
## 2026-02-12 - Layout Re-renders from High-Frequency State
**Learning:** Placing high-frequency state updates (like a `setInterval` clock ticking every second) in a large parent layout component (like `AdminShell`) causes the entire layout and potentially its children to re-render constantly, burning CPU cycles unnecessarily.
**Action:** Isolate high-frequency state updates into separate, small child components so only those small pieces of the DOM update.
