...
## 2026-03-27 - Privilege Escalation via Supabase user_metadata
**Vulnerability:** The `isAdminUser` authorization function checked `user.user_metadata?.role` to determine if a user had admin privileges.
**Learning:** In Supabase, `user.user_metadata` is completely editable by the user via the client-side Auth API (`supabase.auth.updateUser()`). Trusting it for authorization creates a trivial privilege escalation path.
**Prevention:** Never use `user_metadata` for authorization or roles. Rely exclusively on `app_metadata` (which strictly requires a service role key to modify) or server-configured environment variables.
