...
## 2026-03-05 - Privilege Escalation via user_metadata
**Vulnerability:** The `isAdminUser` function in `src/lib/admin-auth.ts` checked `user.user_metadata?.role` to authorize admin access.
**Learning:** In Supabase, `user.user_metadata` can be arbitrarily modified by the user via the client-side Auth API (`supabase.auth.updateUser()`), allowing any user to grant themselves admin privileges.
**Prevention:** Removed `user_metadata` authorization checks. Authorization logic must rely strictly on `app_metadata` (which requires a service role key to modify) or server-side configurations.
