...
## 2026-03-24 - Privilege Escalation via Supabase user_metadata
**Vulnerability:** The `isAdminUser` function in `src/lib/admin-auth.ts` checked `user.user_metadata?.role` to determine if a user had admin privileges.
**Learning:** In Supabase, `user.user_metadata` is editable by the user themselves via the client-side Auth API (`supabase.auth.updateUser()`). It must never be used for authorization or role checks, as doing so creates a trivial privilege escalation vulnerability.
**Prevention:** Use `app_metadata` instead, which strictly requires a service role key to modify. Never trust `user_metadata` for security-sensitive logic.
