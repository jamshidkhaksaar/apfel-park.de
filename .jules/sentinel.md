...
## 2026-04-01 - User Metadata Authorization Bypass
**Vulnerability:** Checking `user.user_metadata` for roles in `src/lib/admin-auth.ts`.
**Learning:** In Supabase, `user.user_metadata` is editable by the user themselves via the client-side Auth API (`supabase.auth.updateUser()`). Using it for authorization creates a privilege escalation vulnerability where a standard user could modify their own metadata to grant themselves admin access.
**Prevention:** Never use `user_metadata` for authorization or role checks. Always use `app_metadata` instead, which strictly requires a service role key to modify.
