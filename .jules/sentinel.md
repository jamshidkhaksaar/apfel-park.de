...
## 2026-03-25 - Privilege Escalation via Supabase user_metadata
**Vulnerability:** The `isAdminUser` function in `src/lib/admin-auth.ts` checked `user.user_metadata?.role` to grant administrative access.
**Learning:** In Supabase, `user.user_metadata` is editable by the user themselves via the client-side Auth API (`supabase.auth.updateUser()`), making it untrusted data. Relying on it for authorization creates a critical privilege escalation vulnerability.
**Prevention:** Never use `user_metadata` for authorization or role checks. Use `app_metadata` instead, which strictly requires a service role key to modify, or rely on secure server-side configuration like `ADMIN_EMAILS`.
