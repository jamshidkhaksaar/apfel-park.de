...
## 2025-02-19 - Supabase Privilege Escalation via user_metadata
**Vulnerability:** Authorization check in `isAdminUser` was reading from `user.user_metadata?.role`.
...
**Prevention:** Use `app_metadata` instead, which strictly requires a service role key to modify.
