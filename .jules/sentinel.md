...
## 2026-03-04 - Privilege Escalation via user_metadata
**Vulnerability:** The `isAdminUser` authorization function relied on `user.user_metadata?.role` to determine if a user had admin access, allowing users to grant themselves admin privileges via the client-side `supabase.auth.updateUser()` API.
**Learning:** In Supabase, `user.user_metadata` is editable by the user themselves via the client-side Auth API. It must never be used for authorization or role checks.
**Prevention:** Use `app_metadata` instead, which strictly requires a service role key to modify, for any role-based authorization checks.
