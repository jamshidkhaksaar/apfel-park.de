...
## 2026-11-20 - Privilege Escalation via Supabase user_metadata
...
**Learning:** In Supabase, `user.user_metadata` is editable by the user themselves via the client-side Auth API (`supabase.auth.updateUser()`). It must never be used for authorization or role checks, as doing so creates a privilege escalation vulnerability.
**Prevention:** Always use `user.app_metadata` for authorization roles and claims, which strictly requires a service role key to modify.
