...
$allParams = array_merge(
	...
	array(
		...
		array(
			...
			"param_name"  => "show",
			"description" => sprintf( esc_attr__("How many %s item you want to show.", "fixtech"), $team_type_title ),
			"value"       => array(
		...
		array(
			...
			"holder"      => "div",
			"class"       => "",
			"heading"     => sprintf( esc_attr__("Show Sortable %s Links",'fixtech'), $team_group_title ),
			"description" => sprintf( esc_attr__("Show sortable %s links above box items so user can sort by just single click.",'fixtech'), $team_group_title ),
			"param_name"  => "sortable",
			"value"       => array(
		...
		array(
			...
			'param_name'  => 'allword',
			'description' => esc_attr__( 'Replace ALL word in sortable group links. Default is ALL word.', 'fixtech' ),
			"std"         => "All",
			'dependency'  => array(
				'element'   => 'sortable',
				'value'     => array( 'yes' ),
		...
		array(
			...
			"description" => sprintf( esc_attr__("Show pagination links below %s boxes.",'fixtech'), $team_type_title_singular ),
		...
		array(
			"type"        => "checkbox",
			"heading"     => sprintf( esc_attr__("From %s", "fixtech"), $team_group_title_singular ),
			"param_name"  => "category",
			"description" => sprintf( esc_attr__('If you like to show %1$s from selected %2$s than select the category here.', "fixtech"), $team_type_title, $team_group_title ),
			"value"       => $teamGroupList,
			'group'		  => esc_attr__( 'Box Style', 'fixtech' ),
		),
		array(
			...
			"description" => sprintf( esc_attr__("Sort retrieved %s by parameter.",'fixtech'), $team_type_title_singular ),
			"param_name"  => "orderby",
			"value"       => array(
...
vc_map( array(
	"name"     => sprintf( esc_attr__("ThemetechMount %s Box", "fixtech"), $team_type_title_singular ),
	"base"     => "tm-teambox",
